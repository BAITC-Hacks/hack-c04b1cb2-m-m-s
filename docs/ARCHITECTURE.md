...
Решение состоит из обучаемой модели и цикла, который последовательно воспроизводит ежедневные исторические решения. Внешняя LLM и личный API-ключ не нужны. «Агентность» здесь означает, что один запуск сам выбирает допустимый погодный выпуск и модель для даты, получает и проверяет данные, строит прогноз и анализирует результат. Отдельный режим наблюдения самостоятельно проверяет входы заданной исторической даты и повторяет расчёт при изменении их содержимого. Метод не выдаётся за свободно рассуждающего чат-агента.
