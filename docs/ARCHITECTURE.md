...
`run_campaign.py` задаёт цель на последовательный исторический период. `station_profile.py` проверяет профиль двух турбин; оператор определяет координаты и каталог совместимых CSV. Внутри дня агент дополнительно получает инструменты `inspect_station` и `train_model`: читает профиль, выбирает обучение при отсутствии или непригодности модели, затем получает погоду и выпускает прогноз. Подбор новой архитектуры ML и автоматическая калибровка здесь не выполняются. Обучается прежняя проверяемая семья кривых мощности с параметрами из истории выбранной станции.

Для кампании лимит расширен до 12 ответов и 16 инструментов на попытку дня; до двух попыток. Планировщик последовательно меняет дату без ожидания реальных суток, сохраняет состояние после каждого успешного дня и останавливается после неисправленной ошибки. Возобновление проверяет артефакты и применённые входы по хешам. Модель привязана к профилю станции и хешам двух CSV, кеш проверяет координаты. Системная блокировка Linux/macOS запрещает два процесса для одной папки и снимается ОС после падения. Папка по умолчанию включает хеши станции и истории; результаты конкурсного расчёта не заменяются.

Веб-кнопка кампании использует серверный профиль `WIND_STATION_FILE` (по умолчанию `stations/example.json`). Без `WIND_TRAINING_DIR` проход доступен для конкурсной станции на готовых моделях из `models/`: сохраняется их калибровка, рабочие копии привязываются к профилю, а папка и состояние прохода учитывают контрольные суммы моделей. Исходные CSV в этом режиме не нужны. Заданный `WIND_TRAINING_DIR` включает прежнее обучение по истории выбранной станции; некорректный каталог не подменяется готовыми моделями. Веб-режим сохраняет общий с дневным агентом лимит запусков и журнал по идентификатору задачи. После перезапуска сервера оператор повторно запускает проход; завершённые дни пропускаются без LLM. Полный месячный проход и периодический watcher — разные режимы: первый продвигает исторические даты, второй отслеживает изменения входов одной даты.
