...
Веб-кнопка кампании использует только серверные `WIND_STATION_FILE` и `WIND_TRAINING_DIR`, общий с дневным агентом лимит запусков и журнал по идентификатору задачи. После перезапуска сервера оператор повторно запускает проход; завершённые дни пропускаются без LLM. Полный месячный проход и периодический watcher — разные режимы: первый продвигает исторические даты, второй отслеживает изменения входов одной даты.
